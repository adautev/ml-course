...
dat %>% 
	...
	geom_point(aes(x, y)) +
	geom_step(aes(x, y_hat), col = 2)
rf_fit <- train (y ~ ., data = dat, method="Rborist", tuneGrid = data.frame(predFixed = 1, minNode = seq(25, 100, 25)))
plot(rf_fit)
dat %>% 
...
set.seed(1991)
fit_rb<- with(tissue_gene_expression, train(x,y, method="rf", tuneGrid = data.frame(mtry = seq(50, 200, 25)), nodesize=1))
